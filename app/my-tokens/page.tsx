...
import { TokenHolding, TokenNode } from '@/lib/interface';
  ...
  useEffect(() => {
    const fetchHoldings = async () => {
      ...
      try {
        const response = await fetch(
          'https://backpack-api.xnfts.dev/v2/graphql',
          {
            ...
            headers: {
              ...
              cookie:
                '__cf_bm=HoJcVH67CIrMekCr2MvMRhh0jW8BBlSOFtpFJKwrN58-1726043148-1.0.1.1-VArrpIsxdumcWOfr9zoQbk9g3Fr8OZx_iRuDwGGe_6xHqQaYpWWV9Es3UkjV7pkImtWWB3uktfextZaxdyXeOA',
            },
            body: JSON.stringify({
        ...
        const fetchedHoldings = tokenEdges.map(
          ({ node }: { node: TokenNode }) => ({
            address: node.token,
        ...
        toast.error('Failed to fetch token holdings. Please try again.');
      } finally {
        setIsLoading(false);
      }
    };
      ...
      {!publicKey ? (
        ...
        >
          <p className="text-xl text-gray-300 mb-4">
            Please connect your wallet to view your token holdings.
          </p>
