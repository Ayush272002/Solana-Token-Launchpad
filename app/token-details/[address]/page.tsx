...
import { containerVariants, itemVariants } from '@/lib/styles';
import { TokenDetails } from '@/lib/interface';
    ...
    const fetchTokenDetails = async () => {
      ...
      try {
        ...
        if (metadata) {
          ...
          if (metadata.uri) {
            try {
              const metadataJson = await fetch(metadata.uri).then((res) =>
                res.json()
              );
        ...
        toast('Failed to fetch token details. Please try again.');
      } finally {
        setIsLoading(false);
      }
    };
  ...
  return (
    ...
    >
      ...
      {isLoading ? (
        ...
        >
          <motion.div
            className="flex flex-col md:flex-row items-center mb-6 md:mb-8"
            variants={itemVariants}
          >
          ...
          >
            <motion.div variants={itemVariants}>
              ...
              <p className="text-gray-300 break-all text-sm md:text-base">
            ...
            <motion.div variants={itemVariants}>
              ...
              <p className="text-gray-300 text-sm md:text-base">
                {(
                  tokenDetails.totalSupply / Math.pow(10, tokenDetails.decimals)
                ).toLocaleString()}{' '}
                {tokenDetails.symbol}
              </p>
            </motion.div>
            ...
            <motion.div variants={itemVariants}>
              ...
              <p className="text-gray-300 break-all text-sm md:text-base">
                {tokenDetails.mintAuthority || 'None (Fixed Supply)'}
              </p>
            </motion.div>
            <motion.div variants={itemVariants}>
            ...
            <motion.div variants={itemVariants}>
              ...
              <p className="text-gray-300 text-sm md:text-base">
                {tokenDetails.isInitialized ? 'Yes' : 'No'}
              </p>
            </motion.div>
            <motion.div variants={itemVariants}>
              <h3 className="text-lg font-semibold mb-1">Supply Type</h3>
              <p className="text-gray-300 text-sm md:text-base">
                {tokenDetails.fixedSupply ? 'Fixed' : 'Variable'}
              </p>
            </motion.div>
            {tokenDetails.uri && (
              <motion.div variants={itemVariants}>
